...
exec(char *path, char **argv)
{
  ...
  int sh_cmp_res = strncmp("sh", path,2);
  int init_cmp_res = strncmp("/init", path, 5);
  int cow1_cmp_res = strncmp("test8-cow1", path, 10);
  int cow2_cmp_res = strncmp("test9-cow2", path, 10);
  int cow3_cmp_res = strncmp("test10-cow3", path, 11);
  
  if(((sh_cmp_res == 0)) || (init_cmp_res == 0) || (cow1_cmp_res == 0) || (cow2_cmp_res == 0) || (cow3_cmp_res == 0)){
    p->ondemand = false;
  }else{
    p->ondemand = true;
  }
  
  /*Intialization for cow*/
  cow_init();
  p->cow_enabled = 0;
  p->cow_group = -1;
  ...
  for(i=0, off=elf.phoff; i<elf.phnum; i++, off+=sizeof(ph)){
    ...
    /* Check if ondemand paging enable then skip loading */
    if(p->ondemand == true){
